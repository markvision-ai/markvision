      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row justify-between gap-4">
          <div>
            ...
            <CardDescription>Документы для обучения ИИ и команды</CardDescription>
          </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{new Set(documents.map(d => d.category)).size || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {filteredDocuments.length === 0 ? (
          <Card className="col-span-full border-dashed">
            <CardContent className="py-12 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-1">Здесь будут ваши скрипты для ИИ</p>
            </CardContent>
          </Card>
        ) : (
          filteredDocuments.map((doc) => (
            <Card key={doc.id} className="hover:border-primary/50 transition-colors">
              ...
              <CardContent>
                ...
                <div className="flex gap-2">
                  {!doc.is_ai_trained && (
                    <Button size="sm" variant="outline" onClick={() => handleTrainAI(doc.id)} className="gap-1">
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddDocument} disabled={!newDoc.title || !newDoc.content}>Создать</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>

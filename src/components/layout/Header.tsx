...
export const Header = ({
  ...
  return (
    <header className="h-16 md:h-20 border-b border-white/10 bg-background/60 backdrop-blur-2xl flex items-center justify-between px-3 md:px-8 sticky top-0 z-50 transition-all duration-300 shadow-sm">
        ...
        <div className="relative hidden lg:block global-search-container">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-10 py-2 backdrop-blur-md bg-white/5 border border-white/10 shadow-inner rounded-xl text-sm w-64 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => {
          ...
          <AnimatePresence>
            {showResults && searchResults.length > 0 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute top-full mt-2 w-96 backdrop-blur-2xl bg-background/80 border border-white/10 rounded-2xl shadow-interstellar z-50 max-h-[400px] overflow-hidden"
              >

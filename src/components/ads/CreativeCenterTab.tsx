      ...
      while (attempts < maxAttempts) {
        ...
        if (updatedCommand) {
          if (updatedCommand.status === 'completed' && updatedCommand.result) {
            ...
            throw new Error((updatedCommand as any).error || 'AI generation failed');
          }
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={platform} onValueChange={(v) => setPlatform(v as any)}>
        <TabsList className="w-full justify-start bg-muted/50 dark:bg-muted/30">
          <TabsTrigger value="facebook" className="gap-2">
            <div className="w-4 h-4 bg-[#1877F2] rounded text-[10px] text-white font-bold flex items-center justify-center shadow-sm">f</div>
            Meta Ads
          </TabsTrigger>
          <TabsTrigger value="google" className="gap-2">
            <div className="w-4 h-4 bg-[#EA4335] rounded text-[10px] text-white font-bold flex items-center justify-center shadow-sm">G</div>
            Google Реклама
          </TabsTrigger>
          <TabsTrigger value="tiktok" className="gap-2">
            <div className="w-4 h-4 bg-black rounded text-[10px] text-white font-bold flex items-center justify-center shadow-sm">T</div>
            TikTok
          </TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-white/50 bg-white/5 dark:bg-white/10 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={3}
                className="bg-white/5 dark:bg-muted/20 text-foreground border-white/50"
              />
            </div>
            ...
            >
              ...
              Сгенерировать для {platform === 'facebook' ? 'Meta' : platform === 'google' ? 'Google' : 'TikTok'}
            </Button>
                ...
                <div className="space-y-4">
                  ...
                  {generatedContent.headlines.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {generatedContent.headlines.map((headline, i) => (
                        <div
                          key={i}
                          className="flex items-center justify-between p-2 bg-white/5 dark:bg-muted/20 rounded-lg group border border-white/50"
                        >
                  ...
                  {generatedContent.descriptions.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {generatedContent.descriptions.map((desc, i) => (
                        <div
                          key={i}
                          className="flex items-start justify-between p-2 bg-white/5 dark:bg-muted/20 rounded-lg group border border-white/50"
                        >
                  ...
                  {generatedContent.primaryText && (
                    <div className="space-y-2">
                      ...
                      <div className="p-3 bg-white/5 dark:bg-muted/20 rounded-lg group relative border border-white/50">
        ...
        <Card className="border-white/50 bg-white/5 dark:bg-white/10 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col items-center">
              <div
                className={`relative bg-white/10 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 rounded-lg overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 ${previewFormat === 'feed'
                  ? 'w-full aspect-[4/5]'
                  : 'w-[200px] aspect-[9/16]'
                  }`}
              >

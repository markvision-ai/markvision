-- Fix agency_metrics_view to support dynamic monthly filtering and accurate CRM lead counting
...
WITH monthly_ads AS (
    ...
    GROUP BY ad_account_id, date_trunc('month', date)::date
),
crm_metrics AS (
    SELECT 
        fb_ad_account_id,
    ...
    GROUP BY fb_ad_account_id, date_trunc('month', created_at)::date
),
combined_metrics AS (
    SELECT 
